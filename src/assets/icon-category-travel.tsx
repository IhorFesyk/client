import { IPropsIcon } from "utils";

export const IconCategoryTravel: React.FC<IPropsIcon> = ({ className }) => {
  return (
    <svg
      version="1.1"
      viewBox="0 0 296.781 296.781"
      className={className}
      stroke="currentColor"
      fill="currentColor"
    >
      <path
        d="M46.881,157.053l-35.494-16.586c-3.012-1.407-6.582-0.805-8.966,1.514c-2.383,2.319-3.083,5.871-1.759,8.921l7.613,17.53
		L46.881,157.053z"
      />
      <path
        d="M110.863,147.703l-90.246,26.598c-3.462,1.021-5.813,4.233-5.736,7.842c0.075,3.608,2.558,6.72,6.061,7.594l97.355,24.304
		c0.637,0.159,1.287,0.238,1.938,0.238c0.758,0,1.516-0.107,2.25-0.323l150.332-44.052c17.724-5.281,27.846-23.998,22.564-41.725
		c-4.207-14.109-17.42-23.965-32.132-23.965c-3.238,0-6.465,0.473-9.567,1.397l-71.005,20.927l-12.478-5.83L90.051,83.256
		c-3.836-1.793-8.404-0.285-10.421,3.438l-5.553,10.252c-1.58,2.916-1.196,6.505,0.964,9.021l29.31,34.149L110.863,147.703z"
      />
    </svg>
  );
};
